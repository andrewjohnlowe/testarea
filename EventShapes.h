...
return result;
  ...
  double/* jStr_evtXX_*/BT1234() const {
double result = NaN;
if(_dijets) {
  double mt = (*_firstJet + *_secondJet + *_thirdJet + *_fourthJet).mt();
  result = Divide(mt, jetSumMT());
}
return result;
}

  double/* jStr_evtXX_*/BTN() const {
double result = NaN;
if(_dijets) {
  double mt = jetSum().mt();
  result = Divide(mt, jetSumMT());
...
CLHEP::HepLorentzVector /* jStr_evtXX_*/jetSum() const { DEBUG(0); // HET
if(_jets.empty()) return CLHEP::HepLorentzVector(NaN, NaN, NaN, NaN); DEBUG(0);
return std::for_each(_jets.begin(), _jets.end(), Add4Vec()).sum;
}
...
private:
  ...
  // Adder functor
  struct Add4Vec: public std::unary_function<CLHEP::HepLorentzVector*, void> {
    Add4Vec(): sum(CLHEP::HepLorentzVector(0., 0., 0., 0.)) {}
    CLHEP::HepLorentzVector sum;
    result_type operator()(argument_type x) { sum += *x; }
  };
