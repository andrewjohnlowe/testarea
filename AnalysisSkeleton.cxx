...
std::string JetContainerName = "AntiKt4TowerJets";
const JetCollection* jetTES;

m_sc=m_storeGate->retrieve( jetTES, JetContainerName);

if( m_sc.isFailure() ) {
  ATH_MSG_ERROR("JetCollection " << JetContainerName << " not found.");
  return StatusCode::SUCCESS;
} else {  
  njtag = (*jetTES).size();
  ATH_MSG_INFO("Jet " << JetContainerName<< " found with " << njtag << " jets.");
}

 // --- iterate over the ParticleJet container 
  ...
  for (; jetItr != jetEnd; ++jetItr) {
    ...
    double w_cmb  = (*jetItr)->getFlavourTagWeight(); // combination of IP3D and SV1, the recommanded tagger

    ATH_MSG_INFO("Jet " << ijet << " with " << ntag
                    << " tags: IP2D weight : "<<w_ip2d
                    << " CMB weight : "<<w_cmb);

    // add code from tutorial section f/ here:
    Jet* myJet=*jetItr;
    const Analysis::BaseTagInfo* myInfo=dynamic_cast<const Analysis::BaseTagInfo*>(myJet->tagInfo("IP3D"));
    if (myInfo) 
    {
      std::vector<double> myProbabilityValues=myInfo->tagLikelihood();
      double probabilityBottomJet=myProbabilityValues[0];
      double probabilityLightJet=myProbabilityValues[1];

      ATH_MSG_INFO("IP3D: likelihood value for probability of having a b-jet: " << probabilityBottomJet);
      ATH_MSG_INFO("IP3D: likelihood value for probability of having a light jet: " << probabilityLightJet );
    }
    // add code from tutorial section g/ here:
    ...
    if(mcinfo) {
      ...
      ATH_MSG_INFO("could not find TruthInfo for matching jet");
    }
    int iflav(0);
    ...
    ATH_MSG_INFO("Label : " << iflav);

    // add code from tutorial section h/ here:
